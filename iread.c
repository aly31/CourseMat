...
int main (int argc, char* argv[])
{
  ...
  MPI_File_iread(fha,v1,counts[my_rank],MPI_DOUBLE,&req[0]);
  MPI_File_iread(fhb,v2,counts[my_rank],MPI_DOUBLE,&req[1]);

  // go off and do other work while the filesystem request is being done....

  MPI_Waitall(2,req,statuses);  // wait for filesystem request to complete
